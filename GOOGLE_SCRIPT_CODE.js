...
function doPost(e) {
  ...
  try {
    ...
    if (data.action === 'getRequisitions') {
      var sheet = doc.getSheetByName('Requisicoes');
      if (!sheet) return responseJSON({ status: 'success', requisitions: [] });
      var rows = sheet.getDataRange().getValues();
      var result = [];
      for (var i = 1; i < rows.length; i++) {
        try { result.push(JSON.parse(rows[i][4])); } catch (e) {}
      }
      return responseJSON({ status: 'success', requisitions: result.reverse() });
    }
...
function doGet(e) {
  ...
  if (action === 'getUsers') {
    var sheetUsers = doc.getSheetByName('Usuarios');
    if (!sheetUsers) return responseJSON([]);
    var rows = sheetUsers.getDataRange().getValues();
    var users = [];
    for (var i = 1; i < rows.length; i++) {
      users.push({ username: rows[i][0], name: rows[i][2], role: rows[i][3] });
  ...
  if (action === 'getImage' && e.parameter.fileId) {
    try {
